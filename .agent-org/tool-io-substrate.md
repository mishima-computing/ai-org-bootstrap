...
| `detect-ecosystem-profiles.py` | controller-run | `--self-test` | conforming stdout selector JSON or artifact write; self-test emits status plus advisory uncarded profile ids | conforming 0/1 | Keep self-test wired through the bootstrap pack validator while functional CI runs that validator | Next selector change |
...
| `select-review-precedents.py` | controller-run | `--self-test` | conforming stdout selector JSON; self-test emits status plus fixture errors | conforming 0/1 | Selection key is supplied review-time lens/NN class only, not repo facts | Next precedent selector change |
