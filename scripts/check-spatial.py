...
class ChromeUnavailableError(SpatialError):
    pass
...
(function () {{
  ...
  function publish(done) {{
    ...
    const facts = {{
      ...
      frames: frames,
      notes: ["All declared widths are simulated through iframes in one Chrome launch."]
    }};
...
def chrome_run(
    ...
    """Invoke Chrome once and return dump-dom stdout.

    All Chrome flags are constructed here so security review has one subprocess
    boundary. No shell is used; no sandbox-disabling or remote-debugging flag is
    added by default.
    """
    ...
    if completed.returncode != 0:
        ...
        raise ChromeUnavailableError(f"Chrome dump-dom failed: {detail[:500]}")
    return completed.stdout


def with_probe_width(url: str, width: int) -> str:
    separator = "&" if "?" in url else "?"
    return f"{url}{separator}{urllib.parse.urlencode({'__spatial_probe_width': str(width)})}"
        ...
        def visit_Call(self, node: ast.Call) -> Any:
            ...
            if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name):
                if func.value.id == "subprocess":
                    calls.append(stack[-1] if stack else "<module>")
            self.generic_visit(node)

    Visitor().visit(tree)
    if calls != ["chrome_run"]:
        raise AssertionError(f"subprocess calls must be isolated to chrome_run, found {calls}")
